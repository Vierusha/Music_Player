package com.codex.musicplayer.fragments;
...
import com.codex.musicplayer.activities.FolderDialog;
import com.codex.musicplayer.adapter.AccentAdapter;
import com.codex.musicplayer.helper.ThemeHelper;
import com.codex.musicplayer.model.Folder;
import com.codex.musicplayer.player.PlayerManager;
import com.codex.musicplayer.viewmodel.MainViewModel;
import com.codex.musicplayer.viewmodel.MainViewModelFactory;
...
public class SettingsFragment extends Fragment implements View.OnClickListener {
        ...
        toolbar.setOnMenuItemClickListener(item -> {
            ...
            if (id == R.id.fb) {
                startActivity(new Intent(
                        Intent.ACTION_VIEW,
                        Uri.parse(MPConstants.FACEBOOK_REPO_URL)

                )); return true;
            }
            return false;
        });
        toolbar.setNavigationOnClickListener(v -> requireActivity().recreate());
    }
    ...
    private void setCurrentThemeMode() {
        ...
        if (mode == AppCompatDelegate.MODE_NIGHT_NO)
            currentThemeMode.setImageResource(R.drawable.light_mode);

        else if (mode == AppCompatDelegate.MODE_NIGHT_YES)
            currentThemeMode.setImageResource(R.drawable.night_mode);

        else
            currentThemeMode.setImageResource(R.drawable.auto_mode);
    }
